...
class OperationNode(BaseNode):
    ...
    def __init__(self):
        ...
        self.create_property('duration', 1, widget_type=4, range=(1, 9999),
                             widget_tooltip='Тривалість операції (у хвилинах)')
        self.create_property('workers_needed', 1, widget_type=4, range=(1, 100),
                             widget_tooltip='Кількість працівників для операції')

        self.add_text_input('op_name', label='Назва', text='Нова операція',
                            tooltip='Назва операції')
        self.add_spinbox('duration', label='Тривалість', tooltip='Тривалість (хв)')
        self.add_spinbox('workers_needed', label='Працівників',
                         tooltip='Кількість працівників')
